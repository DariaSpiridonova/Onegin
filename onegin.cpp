...
int main()
{
    ...
    if(!open_files_success(read_strings, sorted_strings)) return -1; 
    ...
    buffer[num_success_read_symbols + 1] = '\0';
    ...
    for (size_t i = 0; i < num_of_strings_in_file; i++) // проверка содержимого массива указателей
    {
        printf("%d: ", i);
        printf("<%s>", pointers_on_strings[i]);
    }

    char **sources_pointers_on_strings = pointers_on_strings;

    sort_array_of_points(pointers_on_strings, num_of_strings_in_file, my_strcmp_from_the_begining);
    for (size_t i = 0; i < num_of_strings_in_file; i++) // проверка содержимого массива указателей
    {
        printf("%d: ", i);
        printf("<%s>", pointers_on_strings[i]);
    ...
    write_data_for_a_file(pointers_on_strings, num_of_strings_in_file, sorted_strings);
    
    sort_array_of_points(pointers_on_strings, num_of_strings_in_file, my_strcmp_from_the_end);
    for (size_t i = 0; i < num_of_strings_in_file; i++) // проверка содержимого массива указателей
    {
        printf("%d: ", i);
        printf("<%s>", pointers_on_strings[i]);
    ...
    /*qsort(pointers_on_strings, num_of_strings_in_file, sizeof(char *), compare_str_from_the_begining);
    write_data_for_a_file(pointers_on_strings, num_of_strings_in_file, sorted_strings);

    qsort(pointers_on_strings, num_of_strings_in_file, sizeof(char *), compare_str_from_the_end);
    write_data_for_a_file(pointers_on_strings, num_of_strings_in_file, sorted_strings);*/

    write_data_for_a_file(sources_pointers_on_strings, num_of_strings_in_file, sorted_strings);

    if (fclose(read_strings) == EOF) return -1;
    if (fclose(sorted_strings) == EOF) return -1;
...
void add_pointers_on_strings(char *buffer, char **pointers_on_strings, size_t num_of_strings_in_file)
{
    ...
    for (; i < (num_of_strings_in_file - 1) && pointer_on_new_string != NULL; i++)
    {
        *pointer_on_new_string = '\0';
        if (*(pointer_on_new_string - 1) == '\r') *(pointer_on_new_string - 1) = '\0';
